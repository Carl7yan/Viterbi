...
#create_clock [get_ports dst_clk] -name CLK1_NAME -period $CLK1_SCALE_PERIOD
...
#set_input_delay  -max [expr $CLK0_SCALE_PERIOD * $INPUT_DELAY_SCALE_RATIO] -clock CLK0_NAME [get_ports {din}]
#set_input_delay  -max [expr $CLK1_SCALE_PERIOD * $INPUT_DELAY_SCALE_RATIO] -clock CLK1_NAME [get_ports {dst_rst_n}]
set_input_delay  -max [expr $CLK0_SCALE_PERIOD * $INPUT_DELAY_SCALE_RATIO] -clock V_CLK0_NAME [remove_from_collection [all_inputs] [get_ports {*clk* *rst*}]]
#set_input_delay  -min 0  -clock V_CLK0_NAME [remove_from_collection [all_inputs] [get_ports {*clk* *rst*}]]
#set_input_delay  -min 0 -clock CLK0_NAME [get_ports {src_rst_n din}]

#set_input_delay  -max [expr $CLK0_SCALE_PERIOD * $INPUT_DELAY_SCALE_RATIO] [get_ports {src_rst_n}]
#set_input_delay  -min 0 [get_ports {src_rst_n}]
#set_input_delay  -max [expr $CLK1_SCALE_PERIOD * $INPUT_DELAY_SCALE_RATIO] [get_ports {dst_rst_n}]
#set_input_delay  -min 0 [get_ports {dst_rst_n}]


#set_output_delay -min 0 -clock CLK0_NAME [all_outputs]
set_output_delay -max [expr $CLK0_SCALE_PERIOD * $OUTPUT_DELAY_SCALE_RATIO] -clock CLK0_NAME [all_outputs]
